...
class _VenueDetailScreenState extends ConsumerState<VenueDetailScreen> {
  ...
  // LEARN: the screen's data comes from the family provider keyed by this
  // venue's id. These getters ref.READ the current value — callbacks
  // (share, directions) need values, not subscriptions; build() below does
  // the one ref.WATCH that keeps the widget live. The fallback chain is
  // the whole degraded-state design: loading and error both render the
  // complete list payload (initialVenue, no photos); data upgrades it.
  AsyncValue<VenueDetail> get _detail =>
      ref.read(venueDetailControllerProvider(widget.initialVenue.id));
  Venue get _venue => _detail.value?.venue ?? widget.initialVenue;
  List<VenuePhoto> get _photos =>
      _detail.value?.photos ?? const <VenuePhoto>[];
  ...
  Widget build(BuildContext context) {
    // Subscribe: any state change of this venue's detail (loading -> data,
    // a refresh, an error) rebuilds; values are read through the getters.
    ref.watch(venueDetailControllerProvider(widget.initialVenue.id));
